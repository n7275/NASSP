...
void FCell::UpdateFlow(double dt)
{
	...
	Q_N2_Blanket = (N2_Blanket->mass*N2_Blanket->c)*
		...
		(1 - exp(-(2.5 * dt) / (N2_Blanket->mass*N2_Blanket->c))); //analytical heat transfer model, replaces old E�ler's method model
	...
	Q_N2_StorageTank = (N2_storageTank->mass*N2_storageTank->c)*
		...
		(1 - exp(-(0.8 * dt) / (N2_storageTank->mass*N2_storageTank->c))); //analytical heat transfer model, replaces old E�ler's method model
	...
	Q_O2_Source = (O2_SRC->parent->mass*O2_SRC->parent->c)*
		...
		(1 - exp(-(O2ChamberHeatTransferCoeff * dt) / (O2_SRC->parent->mass*O2_SRC->parent->c))); //analytical heat transfer model, replaces old E�ler's method model
	...
	Q_H2_Source = (H2_SRC->parent->mass*H2_SRC->parent->c)*
		...
		(1 - exp(-(H2ChamberHeatTransferCoeff * dt) / (H2_SRC->parent->mass*H2_SRC->parent->c))); //analytical heat transfer model, replaces old E�ler's method model
...
void Cooling::refresh(double dt) 
{
	...
	for (int i = 0; i <= nr_activelist - 1; i++)
	{
		heat_ex = (activelist[i]->mass*activelist[i]->c)*
			...
			(1 - exp(-(activelength[i] * active_h[i] * isolation * dt) / (activelist[i]->mass*activelist[i]->c))); //analytical heat transfer model, replaces old E�ler's method model
