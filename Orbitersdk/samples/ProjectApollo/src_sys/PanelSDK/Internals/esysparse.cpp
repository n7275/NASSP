...
void E_system::Create_FCell(char *line) {
	...
	sscanf(line+7, "%s %i <%lf %lf %lf> %lf %s %s %s %s %s", name, &status, &pos.x, &pos.y, &pos.z,
		&power, source1, source2, source3, source4, source5);
	...
	h_Tank* CONDENSOR = (h_Tank*)P_hydraulics->GetPointerByString(source5);

	FCell *new_fc = (FCell*)AddSystem(new FCell(name, status, pos, O_SRC, H_SRC, WATER, (float)power, N2, CONDENSOR));
}
